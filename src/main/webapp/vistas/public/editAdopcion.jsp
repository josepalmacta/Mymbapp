...
<div class="contain">
    <div id="mensajeconf" class="text-center d-none">
        ...
        <a href="/adopcion/post/<% out.print(obj.getString("postid")); %>" class="btn btn-warning border-radius-100 btn-block confirmation-btn mt-2 text-primary"><i class="fa-solid fa-file-invoice me-2 text-primary"></i> Ver publicacion</a>
    </div>
    <div id="contenid">
        <div class="p-3">
            ...
            <div class="mb-3 mt-5">
                <h6>¿Ya se concreto la adopcion?</h6>
                <h7>Si ya se concreto la adopcion de la mascota por favor haz clic en el boton para actualizar el estado de la publicacion a "ADOPTADO"</h7>
                <div class="input-group flex-nowrap p-3">
                    <button onclick="mostrarConfirm()" class="btn btn-primary"><i class="fa-solid fa-check"></i> Mascota adoptada</button>
                </div>
            ...
            <div class="mb-3 mt-5">
                <label class="form-label">REQUISITOS PARA LA ADOPCION</label>
                <div class="input-group flex-nowrap">
                    <textarea rows="4" id="requisitos" name="requisitos" class="form-control px-3 py-2"
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex flex-wrap">
                                    ...
                                    <div class="card rounded imgl position-relative m-2 imgc">
                                        <img class="imgl imgn" src="/imgs/posts/a/<% out.print(masco.getJsonArray("imgs").getString(j)); %>">
                                    </div>
                                    <% } %>
                                </div>
                            </div>
...
<script>
    ...
    function mostrarConfirm() {
        swal(
                {
                    type: "warning",
                    text: 'Si actualiza el estado a "ADOPTADO" se dara por finalizada la publicacion.',
                    position: 'center',
    ...
    function valido() {
        if ($("#requisitos").val().trim() === "") {
            mostrarMensaje({"success": false, "estado": "warning", "mensaje": "AGREGUE LOS REQUISITOS DE ADOPCION"});
            return false;
        }
...
<script>
    ...
    function enviarDat() {
        ...
        $.ajax({
            url: "/adopcion/edit",
            data: {post: $("#idpost").val(), contacto: $("#contacto").val(), requisitos: $("#requisitos").val(), lat: map.getCenter()["lat"].toString(), long: map.getCenter()["lng"].toString()},
            type: "POST",
            crossDomain: true,
