 ...
 * @author Usuario
 */
public class postAdopcion {

    private String id, estado, latitud, longitud, contacto, fecha, persona, barrio, requisitos, iddetalle, imgPath, pagina, departamento, especie, raza, ciudad;
    ...
    public postAdopcion(Connection connection) throws Exception {
        conn = connection;
    }
    ...
    public void guardarCabecera(JsonArray arr) throws Exception {
        ...
        rs = st.executeQuery("SELECT coalesce(MAX(id),343962) + 1 as num from adopciones");
        rs.next();
        String nid = rs.getString("num");
        st.executeUpdate("INSERT INTO adopciones(id, sis_estado, estado, latitud, longitud, contacto, fecha, persona, barrio, requisitos) values('" + nid + "', 'PENDIENTE', 'ADOPCION', '" + latitud + "', '" + longitud + "', '" + contacto + "', '" + fecha + "', '" + persona + "', '" + barrio + "', '" + requisitos + "')");
    ...
    public void guardarDetalle(String encon, JsonArray arr) throws Exception {
        ...
        for (int i = 0; i < arr.size(); i++) {
            ...
            st.executeUpdate("INSERT INTO detalleadopcion(idadopcion, idanimal) values('" + encon + "', '" + aid + "')");
            pk.close();
        }
        st.close();
    ...
    public void modificarPost() throws Exception {
        st = conn.createStatement();
        st.executeUpdate("UPDATE adopciones SET contacto='" + contacto + "', requisitos='" + requisitos + "', latitud='" + latitud + "', longitud='" + longitud + "' WHERE id='" + id + "' AND persona='" +  persona + "'");
        st.close();
        out.println("MODIFICADO CORRECTAMENTE");
    }
    ...
    public void finalizarPost() throws Exception {
        st = conn.createStatement();
        st.executeUpdate("UPDATE adopciones SET estado='ADOPTADO' WHERE id='" + id + "' AND persona='" + persona + "'");
        st.close();
        out.println("MODIFICADO CORRECTAMENTE");
    }
    ...
    public void elim() throws Exception {
        ...
        rs = st2.executeQuery("SELECT idanimal FROM detalleadopcion WHERE idadopcion='" + id + "'");
        while (rs.next()) {
        ...
        st.executeUpdate("DELETE FROM detalleadopcion WHERE idadopcion='" + id + "'");
        st.executeUpdate("DELETE FROM adopciones WHERE id='" + id + "'");
      ...
      public void aprobar() throws Exception {
        st = conn.createStatement();
        st.executeUpdate("UPDATE adopciones SET sis_estado='APROBADO' WHERE id='" + id + "'");
        st.close();
        out.println("MODIFICADO CORRECTAMENTE");
    }
    ...
    public JsonArray listar() throws Exception {
        ...
        st = conn.createStatement();
        rs = st.executeQuery("SELECT p.id, p.fecha, p.sis_estado, per.nombre FROM adopciones p INNER JOIN personas per ON p.persona=per.id WHERE NOT p.sis_estado='ELIMINADO';");
    ...
    public JsonArray listarBusqueda() throws Exception {
        ...
        while (rs.next()) {
            ...
            String ssq = "SELECT det.idanimal, a.nombre, a.descripcion, a.genero, r.nombre, e.nombre FROM detalleadopcion det INNER JOIN animales a ON det.idanimal=a.id INNER JOIN razas r ON a.raza=r.id INNER JOIN especies e ON r.especie=e.id WHERE idadopcion='" + postid + "';";
            
            System.err.println(ssq);
            
            
            rr = st2.executeQuery("SELECT det.idanimal, a.descripcion, a.genero, r.nombre, e.nombre FROM detalleadopcion det INNER JOIN animales a ON det.idanimal=a.id INNER JOIN razas r ON a.raza=r.id INNER JOIN especies e ON r.especie=e.id WHERE idadopcion='" + postid + "';");
            ...
            while (rr.next()) {
                ...
                if(contad > 1){   
                    ...
                    if(!generos.contains(rr.getString(3))){
                        generos += ", " + rr.getString(3);
                    }
                    if(!razas.contains(rr.getString(4))){
                        razas += ", " + rr.getString(4);
                    }
                }
                else{
                    especieee = rr.getString(5);
    ...
    public JsonArray listarHome() throws Exception {
        ...
        st = conn.createStatement();
        String sql = "SELECT p.id, p.latitud, p.longitud, b.nombre, c.nombre, p.requisitos, p.fecha, p.contacto, p.estado FROM adopciones p INNER JOIN barrios b ON p.barrio=b.id INNER JOIN ciudades c ON b.ciudad=c.id WHERE p.sis_estado='APROBADO' order by p.id desc limit 8;";
        System.out.println(sql);
        rs = st.executeQuery(sql);
    ...
    public JsonObject listarID() throws Exception {
        ...
        st = conn.createStatement();
        String sql = "SELECT b.nombre, c.nombre, per.nombre, p.latitud, p.longitud, p.requisitos, p.fecha, p.contacto, p.sis_estado, per.imagen, p.id, per.barrio, p.estado FROM adopciones p INNER JOIN personas per ON p.persona=per.id INNER JOIN barrios b ON p.barrio=b.id INNER JOIN ciudades c ON b.ciudad=c.id  WHERE p.id='" + id + "'";
        if(persona != null){
            sql += " AND p.persona='" + persona + "' AND p.estado='ADOPCION'";
        }
        rs = st.executeQuery(sql);
        ...
        rr = st2.executeQuery("SELECT det.idanimal, a.descripcion, a.genero, r.nombre, e.nombre FROM detalleadopcion det INNER JOIN animales a ON det.idanimal=a.id INNER JOIN razas r ON a.raza=r.id INNER JOIN especies e ON r.especie=e.id WHERE idadopcion='" + id + "';");
