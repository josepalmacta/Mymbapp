 ...
 * @author Usuario
 */
@WebServlet(name = "Svsesiones", urlPatterns = {"/session/*"})
public class Svsesiones extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Svsesiones</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Svsesiones at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
        ...
        if (actio.toLowerCase().equals("login")) {
            ...
            try(Connection connection = dataSource.getConnection()) {
                ...
                if (login.login()) {
                    ...
                    sesion.setAttribute("user", login.getUsuario());
                    sesion.setAttribute("id", login.getCodigo());
                    sesion.setAttribute("rol", login.getRol());
                    response.getWriter().write(getRes(true, "success", "REDIRECT"));
                } else {
