 ...
 * @author Usuario
 */
@WebServlet(name = "SvCiudades", urlPatterns = {"/gestionar/ciudades"})
public class Svciudades extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SvCiudades</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SvCiudades at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try(Connection connection = dataSource.getConnection()) {
            ...
            if (opt != null && opt.equals("add")) {
                ...
                request.getRequestDispatcher("/jsp/ciudades.jsp").forward(request, response);
            
            }else if(opt != null && opt.equals("depar")){
                String departamento = request.getParameter("departamento");
                ciu.setDepartamento(departamento);
                response.setContentType("text/plain");
                response.setCharacterEncoding("UTF-8");
