 ...
 * @author Usuario
 */
@WebServlet(name = "PbPostPerdido", urlPatterns = {"/perdidos/*"})
public class PbPostPerdido extends HttpServlet {
    ...
    private void mostrarPost(String value, String pathInfo, HttpServletRequest request, HttpServletResponse response, Connection connection) throws Exception {
        ...
        getServletConfig().getServletContext().getRequestDispatcher("/vistas/public/postPerdido.jsp").forward(request, response);
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (pathInfo == null) {
            getServletConfig().getServletContext().getRequestDispatcher("/vistas/public/buscarPerdidos.jsp").forward(request, response);
            return;
        }
            ...
            if (action.equals("post")) {
                ...
                if (!value.equals("vistas")) {
                    ...
                    if (value.equals("add")) {
                        ...
                        getServletConfig().getServletContext().getRequestDispatcher("/vistas/public/postPerdidos.jsp").forward(request, response);
                        return;
                    } else if (esNum(value)) {
                        if (pathParts.length > 3 && pathParts[3].equals("edit")) {
                            ...
                            request.setAttribute("dets", post.listarID());
                            getServletConfig().getServletContext().getRequestDispatcher("/vistas/public/editPerdidos.jsp").forward(request, response);
                            return;
                        }
                        if (pathParts.length > 3 && pathParts[3].equals("cartel")) {
                            if (sesion.getAttribute("login") == null) {
                                response.sendRedirect("/login");
                                return;
                            }
                            request.setAttribute("postid", value);
                            getServletConfig().getServletContext().getRequestDispatcher("/vistas/public/cartelPerdidos.jsp").forward(request, response);
                            return;
                        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            if (action.equals("add")) {
                ...
                post.setImgPath(request.getSession().getServletContext().getRealPath("imgs/posts/p"));
                post.setBarrio(obj.getString("barrio"));
                post.setContacto(obj.getString("contacto"));
                post.setEstado("PERDIDO");
                ...
                String recom = "0";
                if (obj.getBoolean("recomp")) {
                    recom = obj.getString("recompensa");
                }
                post.setRecompensa(recom);
